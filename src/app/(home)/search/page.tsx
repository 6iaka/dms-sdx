...
const SearchPage = async ({ searchParams }: Props) => {
  ...
  // Only fetch folders if there's no category filter
  const folders = !category ? await folderService.search(query || "") : [];
  ...
  return (
    <main className="flex flex-col gap-4 p-4">
      ...
      {/* Only show folders section if there's no category filter */}
      {!category && (
